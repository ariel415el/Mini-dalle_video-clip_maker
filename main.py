...
def main():
    ...
    parser.add_argument('--sec_per_img', help='How long to show each image', default=2)
    parser.add_argument('--n_lines', help='Limit number of generated lines in the video to faster testing', default=20)
    ...
    for (text, start, end) in texts_and_durations:
        ...
        for j in range(len(images)):
            img = cv2.cvtColor(images[j], cv2.COLOR_RGBA2BGR)
    ...
    # Write video
    video = cv2.VideoWriter(vid_path, 0, args.fps, (256, 256))
    for i, frame in enumerate(frames):
        cv2.imwrite(f"{frames_dir}/frame-{i}.png", img)
        video.write(frame)
